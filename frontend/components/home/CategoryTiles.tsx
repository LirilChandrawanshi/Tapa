...
export function CategoryTiles({
  ...
  return (
    <section className="mx-auto max-w-[1280px] px-4 pt-10 pb-14 md:px-10">
      <SectionHeader
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {tiles.map((t) => (
          <Link
            key={t.title}
            href={t.href}
            className="group flex flex-col rounded-[15px] border border-border bg-card p-5 transition-colors hover:border-cta"
          >
            ...
            <span className="mt-2 mb-3 text-[12px] leading-relaxed text-sub">
              {t.note}
            </span>
        ...
        {kitsLaunched ? (
          ...
          <div className="flex flex-col rounded-[15px] border border-dashed border-border bg-card/60 p-5">
            <span aria-hidden className="mb-2 text-[22px] opacity-70">
              🧺
            </span>
